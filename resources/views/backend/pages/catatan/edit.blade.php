...
@section('content')
    ...
    <div class="main-content-inner">
                        ...
                        <div class="form-group">
                            ...
                            <select name="jadwal" class="form-control" id="" required onchange="$('.form-jadwal').submit()">
                                ...
                                @foreach ($jadwal as $item)
                                    <option value="{{ $item->id }}" {{ $id_jadwal == $item->id ? 'selected' : '' }}>{{ $item->mapel->nama_mapel }} |
                                        Kelas
        ...
        @if ($id_jadwal != null)
            @if (!empty($siswa))
                <div class="row">
                    <form action="{{ route('catatan.update',$data->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12">
                                            ...
                                            <div class="form-group col-md-12">
                                                ...
                                                <input type="text" class="form-control" value="{{ $data->judul }}" name="judul" placeholder="Judul Catatan.." required>
                                            </div>
                                            <div class="form-group col-md-12">
                                                ...
                                                @foreach ($siswa as $index => $item)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <textarea name="catatan[{{ $item->id }}]" class="form-control">{{ $item->catatanDetail->first()->catatan ?? '' }}</textarea>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                            </tbody>
